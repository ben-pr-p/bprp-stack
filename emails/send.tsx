...
>(email: SendEmailProps, subject: string, template: EmailTemplateName, templateProps: EmailTemplateProps) {
  if (!config.SENDGRID_API_KEY) {
  ...
  const html = await render(<EmailComponent {...(templateProps as unknown as any)} />);
